...
def check_model_compatibility(model_path):
    ...
    try:
        ...
        if model_family and model_family.lower() in [
            "llama",
            "mistral",
            "mixtral",
            "gemma",
            ...
            print(
                "   For best results, use with a compatible draft model of the same architecture family"
            )
        else:
            if model_family:
                print(
                ...
                print(
                    "   Examine the model file name and metadata to make a determination"
                )
        ...
        if compatible:
            if "medgemma" in model_path.lower():
                print("\nSpecial notes:")
                print(
                    "- MedGemma models work well with speculative decoding when paired with a matching draft model"
                )
                print(
                    "- For best results, use a smaller MedGemma model as the draft model"
                )

            if re.search(r"q4_k", model_path.lower()):
                print("\nQuantization note:")
                print(
                    "- Q4_K quantization is good for balance of quality and speed with speculative decoding"
                )
...
def main():
    parser = argparse.ArgumentParser(
        description="Check GGUF models for speculative decoding compatibility"
    )
    parser.add_argument(
        "--model_dir",
        type=str,
        default="/Users/derek/.lmstudio/models",
        help="Directory containing GGUF models",
    )
    parser.add_argument(
        "--max_models", type=int, default=5, help="Maximum number of models to check"
    )
    parser.add_argument(
        "--specific_model",
        type=str,
    ...
    if args.specific_model:
        if os.path.exists(args.specific_model) and args.specific_model.endswith(
            ".gguf"
        ):
            check_model_compatibility(args.specific_model)
            print("\nSpeculative Decoding Setup Tips:")
            print(
                "1. For optimal performance, use a smaller model of the same architecture as the draft model"
            )
            print(
                "2. Ensure both models use the same tokenizer or are from the same model family"
            )
            print("3. Set appropriate draft_tokens (8-10 for 13B or smaller models)")
            print(
                "4. Adjust draft_temperature (0.7-0.8 works well) to balance speed and quality"
            )
            return
        else:
            print(
                f"Error: Specified model '{args.specific_model}' does not exist or is not a GGUF file"
            )
            return
    ...
    print("\n" + "=" * 60)
    print(
        f"Summary: {len(compatible_models)}/{min(len(gguf_files), args.max_models)} models are confirmed compatible with speculative decoding"
    )
