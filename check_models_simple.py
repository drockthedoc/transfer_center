    ...
    if model_family:
        ...
        if model_family == "medgemma":
            if model_size != "unknown" and int(model_size) > 7:
                print(
                    f"Recommended draft model: A smaller MedGemma model (e.g., MedGemma-4B)"
                )
            else:
                print(
                    f"This model could serve as a draft model for larger MedGemma models"
                )

        elif model_family == "mistral" or model_family == "mixtral":
            if "mixtral" in filename and "mistral" in filename:
                print(
                    f"Recommended draft model: A Mistral-7B model for this Mixtral model"
                )
            ...
            if model_size != "unknown" and int(model_size) > 13:
                print(
                    f"Recommended draft model: A smaller Llama model (e.g., Llama-7B)"
                )
            else:
                print(
                    f"This model could serve as a draft model for larger Llama models"
                )
...
def main():
    ...
    print("\n" + "=" * 60)
    print(
        f"Summary: {len(compatible_models)}/{len(gguf_files)} models are likely compatible with speculative decoding"
    )
    ...
    print(
        "1. Pair models from the same architecture family (e.g., MedGemma with MedGemma)"
    )
    print("2. Use a smaller model as the draft model for a larger model")
    print(
        "3. Start with 8-10 draft tokens for models up to 13B, fewer for larger models"
    )
