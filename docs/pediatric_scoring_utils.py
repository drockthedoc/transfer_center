...
RESPIRATORY_EFFORT_MAP = {
    'normal': 0, 'none': 0, 
    'mild': 1, 'slight': 1, 'minimal': 1,
    'moderate': 2, 'increased': 2, 'labored': 2,
    'severe': 3, 'significant': 3, 'accessory muscles': 3, 'grunting': 3
}

OXYGEN_THERAPY_MAP = {
    'none': 0, 'room air': 0, 'ra': 0,
    'low flow': 1, 'nasal cannula': 1, 'simple mask': 1, '<30%': 1, '<2l': 1,
    'high flow': 2, 'non-rebreather': 2, 'cpap': 2, 'bipap': 2, '30-40%': 2, '2-5l': 2,
    'ventilator': 3, 'intubated': 3, 'mechanical ventilation': 3, '>40%': 3, '>5l': 3
}

MENTAL_STATUS_MAP = {
    'alert': 0, 'a': 0, 'normal': 0,
    'voice': 1, 'v': 1, 'responds to voice': 1,
    'pain': 2, 'p': 2, 'responds to pain': 2, 
    'unresponsive': 3, 'u': 3, 'unconscious': 3
}

BEHAVIOR_MAP = {
    'playing': 0, 'appropriate': 0, 'normal': 0, 'sleeping': 0,
    'irritable': 1, 'consolable': 1,
    'reduced': 2, 'lethargic': 2, 'confused': 2,
    'unresponsive': 3, 'unconscious': 3
}

HEMODYNAMIC_MAP = {
    'stable': 0, 'normal': 0,
    'compensated': 1, 'borderline': 1,
    'unstable': 2, 'shock': 2, 'decompensated': 2
}
...
def get_age_based_ranges(age_months):
    ...
    for (min_age, max_age), (hr_min, hr_max, rr_min, rr_max) in AGE_BASED_RANGES.items():
...
def create_na_response(score_name, missing_params, subscore_keys, include_interpretation=True, include_action=True):
    ...
    if include_interpretation:
        response["interpretation"] = f"Cannot calculate {score_name}: missing required parameters"
