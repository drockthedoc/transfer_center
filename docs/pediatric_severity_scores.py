...
from docs.pediatric_scoring_utils import (
    get_age_based_ranges, safe_get_from_map, check_missing_params, create_na_response, normalize_to_risk_level,
...
def calculate_pews(age_months=None, respiratory_rate=None, respiratory_effort=None, oxygen_requirement=None, 
    ...
    pews_thresholds = [
        ...
        (4, "Increased monitoring needed", "Increase frequency of observations; consider notifying medical team"),
        (6, "Medical review needed", "Request urgent medical review; consider PICU consult"),
        (999, "Critical situation", "Immediate medical intervention; PICU consult/transfer indicated")
    ]
    ...
    has_critical = (
        respiratory_subscore >= 3 or 
        cardiovascular_subscore >= 3 or 
        behavior_subscore >= 3
    )
...
def calculate_trap(respiratory_support=None, respiratory_rate=None, work_of_breathing=None, 
    ...
    if missing_params:
        response = create_na_response(
            ...
            include_interpretation=False, include_action=False
        )
    ...
    resp_support_map = {
        'room air': 0, 'none': 0, 'ra': 0,
        'nasal cannula': 1, 'nc': 1, 'low flow': 1, 'simple mask': 1,
        'high flow': 2, 'hfnc': 2, 'non-rebreather': 2,
        'cpap': 2, 'bipap': 2, 'niv': 2,
        'ventilator': 3, 'intubated': 3, 'mechanical ventilation': 3, 'et tube': 3
    }
    ...
    if oxygen_saturation is not None:
        try:
            ...
            if spo2 < 85: respiratory_subscore = max(respiratory_subscore, 3)
            elif spo2 < 90: respiratory_subscore = max(respiratory_subscore, 2)
            elif spo2 < 94: respiratory_subscore = max(respiratory_subscore, 1)
    ...
    neuro_map = {
        'normal': 0, 'alert': 0, 'appropriate': 0,
        'altered': 1, 'irritable': 1, 'lethargic': 1, 'confused': 1,
        'severely altered': 2, 'obtunded': 2, 'unresponsive': 2, 'coma': 2, 'seizure': 2
    }
    ...
    access_map = {
        'no': 0, 'none': 0, 'easy': 0,
        'yes': 1, 'difficult': 1, 'challenging': 1,
        'multiple attempts': 2, 'unsuccessful': 2, 'failed': 2
    }
    
    access_subscore = safe_get_from_map(access_difficulty, access_map)
    
    # Calculate total score
    total_score = respiratory_subscore + hemodynamic_subscore + neuro_subscore + access_subscore
    ...
    trap_thresholds = [
        ...
        (999, "High Risk", "Critical care transport team required; continuous monitoring")
    ]
    
    risk_level, team_recommendation = normalize_to_risk_level(total_score, trap_thresholds)
...
def calculate_cameo2(physiologic_instability=None, respiratory_support=None, oxygen_requirement=None, 
    ...
    instability_map = {
        'none': 0, 'stable': 0, 'minimal': 0,
        'mild': 1, 'occasional': 1,
        'moderate': 2, 'frequent': 2,
        'severe': 3, 'constant': 3, 'unstable': 3
    }
    ...
    resp_map = {
        'none': 0, 'room air': 0,
        'low-flow': 1, 'nasal cannula': 1, 'simple mask': 1,
        'high-flow': 2, 'non-rebreather': 2, 'cpap': 2, 'bipap': 2,
        'ventilator': 3, 'intubated': 3, 'oscillator': 3, 'hfov': 3
    }
    ...
    if oxygen_requirement is not None:
        if isinstance(oxygen_requirement, (int, float)):
            ...
            try:
                ...
                o2_map = {
                    'none': 0, 'room air': 0, 'ra': 0, '21': 0, '21%': 0,
                    'low': 1, 'minimal': 1, 'nasal cannula': 1, 
                    'moderate': 2, 'high': 2, 'face mask': 2, 'non-rebreather': 2,
                    'very high': 3, 'critical': 3, '100%': 3
                }
                param_scores['oxygen_requirement'] = o2_map.get(str(oxygen_requirement).lower(), 0)
    else:
    ...
    cv_map = {
        'none': 0, 'stable': 0,
        'fluid bolus': 1, 'maintenance iv': 1, 'single med': 1,
        'multiple meds': 2, 'pressors': 2, 'vasoactive': 2,
        'multiple pressors': 3, 'unstable': 3, 'ecmo': 3, 'vad': 3
    }
    ...
    vitals_map = {
        'q4h': 0, 'q 4 hours': 0, 'routine': 0, 
        'q2h': 1, 'q 2 hours': 1, 'q1h': 2, 'q 1 hour': 2, 'hourly': 2,
        'continuous': 3, 'q15min': 3, 'q 15 minutes': 3
    }
    ...
    level_map = {
        'none': 0, 'minimal': 0, 'low': 0,
        'mild': 1, 'basic': 1, 'simple': 1,
        'moderate': 2, 'intermediate': 2, 'complex': 2,
        'high': 3, 'severe': 3, 'intensive': 3, 'maximum': 3
    }
    
    # Process remaining parameters
    for param in ['intervention_level', 'invasive_lines', 'medication_complexity', 
                 'nursing_dependency', 'care_requirements', 'patient_factors']:
        param_scores[param] = parse_numeric_or_map(all_params[param], level_map, default=0)
    ...
    acuity_level, nursing_recommendation = normalize_to_risk_level(total_score, cameo_thresholds)
...
def calculate_prism3(vitals=None, labs=None, age_months=None, ventilated=False):
    ...
    subscore_keys = ["cardiovascular", "neurological", "acid_base", "hematologic", "other"]
    
    # Check if vital signs dictionary is missing entirely
    if vitals is None:
        response = create_na_response("PRISM III", ["vitals"], subscore_keys, include_action=False)
        response["mortality_risk"] = "Cannot calculate: vital signs missing"
        return response
    
    # Check if lab values dictionary is missing entirely
    if labs is None:
        response = create_na_response("PRISM III", ["labs"], subscore_keys, include_action=False)
    ...
    missing_vitals = [vital for vital in required_vitals if vital not in vitals or vitals[vital] is None]
    
    # Check for minimum required lab values
    required_labs = ['ph', 'po2', 'glucose', 'potassium']
    missing_labs = [lab for lab in required_labs if lab not in labs or labs[lab] is None]
    
    # If critical parameters are missing, return N/A
    if missing_vitals or missing_labs:
        response = create_na_response("PRISM III", missing_vitals + missing_labs, subscore_keys, include_action=False)
        response["mortality_risk"] = "Cannot calculate: missing critical parameters"
        return response
    
    # Check if age is missing
    if age_months is None:
        response = create_na_response("PRISM III", ["age_months"], subscore_keys, include_action=False)
    ...
    if age_months < 12:  # Infant
        if sbp < 40: cardiovascular_score += 7
        elif sbp < 55: cardiovascular_score += 3
        
        if hr < 90: cardiovascular_score += 3
        elif hr > 160: cardiovascular_score += 3
    elif age_months < 144:  # Child (1-12 years)
        if sbp < 50: cardiovascular_score += 7
        elif sbp < 65: cardiovascular_score += 3
        
        if hr < 70: cardiovascular_score += 3
        elif hr > 150: cardiovascular_score += 3
    else:  # Adolescent (> 12 years)
        if sbp < 60: cardiovascular_score += 7
        elif sbp < 75: cardiovascular_score += 3
        
        if hr < 55: cardiovascular_score += 3
        elif hr > 140: cardiovascular_score += 3
    
    # Neurological scoring
    gcs = vitals.get('gcs')
    pupils = vitals.get('pupils')
    
    if gcs <= 8: neurological_score += 5
    elif gcs <= 11: neurological_score += 2
    ...
    if ph < 7.0: acid_base_score += 6
    elif ph < 7.3: acid_base_score += 2
    elif ph > 7.55: acid_base_score += 3
    
    if pco2 is not None:
        if pco2 > 75: acid_base_score += 3
    
    if ventilated:  # Different PO2 thresholds if ventilated
        if po2 < 42: acid_base_score += 6
        elif po2 < 50: acid_base_score += 3
    else:
        if po2 < 50: acid_base_score += 6
        elif po2 < 60: acid_base_score += 3
    ...
    if glucose > 200: other_score += 2
    elif glucose < 40: other_score += 8
    
    if potassium > 6.9: other_score += 3
    elif potassium < 3.0: other_score += 3
    
    # Age-based thresholds for creatinine
    if creatinine is not None:
        if age_months < 12 and creatinine > 0.85: other_score += 2
        elif age_months < 144 and creatinine > 0.90: other_score += 2
        elif creatinine > 1.30: other_score += 2
    
    if bun is not None:
        if bun > 40: other_score += 3
    ...
    if wbc is not None:
        if wbc < 3.0: hematologic_score += 4
    
    if pt is not None and ptt is not None:
        if pt > 22 or ptt > 57: hematologic_score += 3
    
    if platelets is not None:
        if platelets < 50: hematologic_score += 4
        elif platelets < 100: hematologic_score += 2
    
    # Calculate total score
    total_score = (
        cardiovascular_score + neurological_score + 
        acid_base_score + hematologic_score + other_score
    )
...
def calculate_queensland_non_trauma(resp_rate=None, HR=None, mental_status=None, SpO2=None, age_months=None):
    ...
    # Define subscore keys for consistent NA responses
    subscore_keys = ["respiratory_rate", "heart_rate", "mental_status", "oxygen_saturation"]
    
    missing_params = [param for param, value in required_params.items() if value is None]
    if missing_params:
        response = create_na_response("Queensland Non-Trauma", missing_params, subscore_keys)
    ...
    queensland_thresholds = [
        ...
        (999, "Critical Risk", "Immediate medical intervention; ICU consult/transfer indicated")
    ]
...
def calculate_queensland_trauma(mechanism=None, consciousness=None, airway=None, breathing=None, circulation=None):
    ...
    mechanism_map = {
        'low': 0, 'minor': 0, 'fall < 1m': 0, 'low velocity': 0,
        'medium': 1, 'moderate': 1, 'fall 1-3m': 1, 'moderate velocity': 1,
        'high': 2, 'severe': 2, 'fall > 3m': 2, 'high velocity': 2, 'pedestrian': 2,
        'very high': 3, 'extreme': 3, 'ejection': 3, 'fatality at scene': 3
    }
    ...
    # Score airway status
    airway_map = {
        'patent': 0, 'clear': 0, 'normal': 0,
        'maintainable': 1, 'needs positioning': 1, 'partial obstruction': 1,
        'unmaintainable': 2, 'threatened': 2, 'obstructed': 2, 'intervention needed': 2
    }
    ...
    breathing_map = {
        'normal': 0, 'unlabored': 0, 'good air entry': 0,
        'distressed': 1, 'moderate distress': 1, 'increased work': 1,
        'severely compromised': 2, 'severe distress': 2, 'poor air entry': 2, 'cyanotic': 2
    }
    ...
    circulation_map = {
        'normal': 0, 'good perfusion': 0, 'capillary refill < 2s': 0,
        'reduced': 1, 'delayed capillary refill': 1, 'tachycardia': 1,
        'severely compromised': 2, 'poor perfusion': 2, 'hypotension': 2, 'shock': 2
    }
    ...
    # Define risk thresholds
    trauma_thresholds = [
        (2, "Low Risk", "Consider discharge after appropriate observation and management"),
        (5, "Moderate Risk", "Admit for observation and management; consider transfer if facilities limited"),
        (8, "High Risk", "Stabilize and consider transfer to higher level of care"),
        (999, "Critical Risk", "Immediate resuscitation and transfer to pediatric trauma center")
    ]
...
def calculate_tps(respiratory_status=None, circulation_status=None, neurologic_status=None):
    ...
    if len(missing_params) == 3:  # All parameters missing
        return create_na_response("TPS", missing_params, subscore_keys, include_interpretation=False)
    ...
    resp_map = {
        ...
        'moderate': 2, 'significant': 2, 'distress': 2,
        'severe': 3, 'critical': 3, 'respiratory failure': 3
    }
    
    subscores["respiratory"] = 0 if respiratory_status is None else safe_get_from_map(respiratory_status, resp_map)
    
    # Score circulation status
    circ_map = {
        'normal': 0, 'stable': 0, 'good perfusion': 0,
        'mild': 1, 'compensated': 1, 'tachycardia': 1,
        'moderate': 2, 'hypotension': 2, 'poor perfusion': 2,
        'severe': 3, 'decompensated': 3, 'shock': 3
    }
    
    subscores["circulation"] = 0 if circulation_status is None else safe_get_from_map(circulation_status, circ_map)
    
    # Score neurologic status
    neuro_map = {
        'normal': 0, 'alert': 0, 'appropriate': 0,
        'mild': 1, 'irritable': 1, 'agitated': 1, 'altered': 1,
        'moderate': 2, 'lethargic': 2, 'confused': 2, 'decreased responsiveness': 2,
        'severe': 3, 'unresponsive': 3, 'seizure': 3, 'coma': 3
    }
    
    subscores["neurologic"] = 0 if neurologic_status is None else safe_get_from_map(neurologic_status, neuro_map)
    ...
    tps_thresholds = [
        ...
        (999, "High Risk", "Critical care transport team required; continuous intensive monitoring")
    ]
