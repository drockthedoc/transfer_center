...
from src.core.models import HospitalCampus, PatientData, CampusExclusion as Exclusion

logger = logging.getLogger(__name__)


def check_patient_exclusions(patient: PatientData, campus: HospitalCampus) -> List[Exclusion]:
    """
    ...
    for exclusion in campus.exclusions:
        ...
        if exclusion.min_age is not None and age is not None:
            if age < exclusion.min_age:
                ...
                logger.info(f"Patient excluded due to minimum age requirement: {exclusion.name}")
                continue
        
        if exclusion.max_age is not None and age is not None:
            if age > exclusion.max_age:
                exclusions.append(exclusion)
                logger.info(f"Patient excluded due to maximum age requirement: {exclusion.name}")
                continue
        ...
        if exclusion.min_weight is not None and weight is not None:
            if weight < exclusion.min_weight:
                exclusions.append(exclusion)
                logger.info(f"Patient excluded due to minimum weight requirement: {exclusion.name}")
                continue
        
        if exclusion.max_weight is not None and weight is not None:
            if weight > exclusion.max_weight:
                exclusions.append(exclusion)
                logger.info(f"Patient excluded due to maximum weight requirement: {exclusion.name}")
                continue
        
        # Care level restrictions
        if exclusion.excluded_care_levels and care_level in exclusion.excluded_care_levels:
            exclusions.append(exclusion)
        ...
        if exclusion.excluded_conditions:
            ...
            if patient.extracted_data and "clinical_info" in patient.extracted_data:
                ...
                if "diagnoses" in clinical_info and isinstance(clinical_info["diagnoses"], list):
            ...
            for condition in patient_conditions:
                ...
                for excluded_condition in exclusion.excluded_conditions:
                    if excluded_condition.lower() in condition_lower:
                        exclusions.append(exclusion)
                        logger.info(f"Patient excluded due to condition: {exclusion.name}")
                        break
