...
from src.core.models import HospitalCampus, Location, TransportMode, WeatherData
...
def evaluate_transport_options(
    ...
    if travel_times:
        ...
        logger.info(
            f"Best transport mode to {campus.name}: {best_mode}, {best_time:.1f} minutes"
        )
...
def calculate_total_transport_time(
    ...
    logger.info(
        f"Total transport time ({transport_mode}): {total_time:.1f} minutes "
