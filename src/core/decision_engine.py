...
from src.utils.travel_calculator import get_road_travel_info, get_air_travel_info, Location
...
def recommend_campus(
    ...
    try:
        ...
        for campus_data in campuses_with_beds:
            ...
            if transport_time_estimates and campus.campus_id in transport_time_estimates:
                ...
                if TransportMode.GROUND_AMBULANCE in available_transport_modes:
                    if "ground" in campus_estimates and "duration_minutes" in campus_estimates["ground"]:
                ...
                if TransportMode.AIR_AMBULANCE in available_transport_modes:
                    if "air" in campus_estimates and "duration_minutes" in campus_estimates["air"]:
            ...
            if min_travel_time == float('inf'):
                ...
                if TransportMode.AIR_AMBULANCE in available_transport_modes:
                    try:
                        air_info = get_air_travel_info(sending_facility, campus.location, current_weather)
            ...
            # If still no valid travel time, use a simple distance-based calculation as fallback
            if min_travel_time == float('inf'):
                try:
                    ...
                    print(f"  Fallback calculation: {distance_km:.1f} km, ~{time_minutes:.1f} minutes by ground")
                    min_travel_time = time_minutes
                    best_mode = TransportMode.GROUND_AMBULANCE
                except Exception as e:
                    print(f"  Error in fallback calculation: {e}")
                    # Last resort - assign an arbitrary travel time (4 hours) to ensure we have a result
            ...
            if best_mode is None:
                print(f"  WARNING: No travel mode selected for {campus.name} despite fallbacks")
        ...
        notes = [
            ...
            f"Bed type: {closest_campus_data['bed_type']}, {closest_campus_data['beds_available']} available",
            f"Transport mode: {closest_campus_data['transport_mode']}, travel time: {closest_campus_data['travel_time_minutes']:.1f} minutes",
        ...
        try:
            ...
            explanation = {
                "summary": f"Selected {campus.name} as the closest suitable campus with available {closest_campus_data['bed_type']} beds.",
                "reasons": [
                    "Passed all exclusion criteria checks",
                    f"Has {closest_campus_data['beds_available']} {closest_campus_data['bed_type']} beds available",
                    f"Travel time of {closest_campus_data['travel_time_minutes']:.1f} minutes is the shortest among eligible options"
                ],
                "travel_details": {
            ...
            print(f"Recommended: {campus.name} via {closest_campus_data['transport_mode']}")
            sys.stdout.flush()
    ...
    print(f"DEBUG: Found {len(eligible_campuses)} campuses that passed exclusion checks")
    ...
    for campus_data in campuses_with_beds:
        ...
        if TransportMode.GROUND_AMBULANCE in available_transport_modes:
            try:
                ...
                if road_info and "duration_minutes" in road_info:
                    travel_times[TransportMode.GROUND_AMBULANCE] = road_info["duration_minutes"]
                    print(f"DEBUG: Ground travel time: {road_info['duration_minutes']} minutes")
            except Exception as e:
        ...
        if TransportMode.AIR_AMBULANCE in available_transport_modes:
            try:
                ...
                if air_info and "duration_minutes" in air_info:
                    travel_times[TransportMode.AIR_AMBULANCE] = air_info["duration_minutes"]
                    print(f"DEBUG: Air travel time: {air_info['duration_minutes']} minutes")
            except Exception as e:
    ...
    print(f"DEBUG: Found {len(campuses_with_distance)} campuses with valid travel routes")
    
    # Sort by travel time (closest first)
    campuses_with_distance.sort(key=lambda x: x["travel_time_minutes"])
    print(f"DEBUG: Sorted campuses by travel time: {[(c['campus'].name, c['travel_time_minutes']) for c in campuses_with_distance]}")
    
    # Select the closest campus
    if campuses_with_distance:
        best_option = campuses_with_distance[0]
        chosen_campus = best_option["campus"]
        print(f"DEBUG: Selected best campus: {chosen_campus.name} with travel time {best_option['travel_time_minutes']} minutes")
    else:
        print("DEBUG: CRITICAL ERROR - No campuses with distance available despite earlier check")
        return None
    ...
    try:
        ...
        recommendation_reason = f"Campus {chosen_campus.name} selected: passed exclusion checks, has {best_option['beds_available']} {best_option['bed_type']} beds available, and is the closest eligible campus at {best_option['travel_time_minutes']:.1f} minutes by {best_option['transport_mode']}."
        ...
        print(f"Recommendation: {chosen_campus.name}. Travel: {best_option['travel_time_minutes']:.1f} min via {best_option['transport_mode']}.")
        return recommendation
    except Exception as e:
