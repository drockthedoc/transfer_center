...
from src.core.models import CampusExclusion, HospitalCampus, PatientData
...
def matches_age_restriction(
    patient_age: Optional[int], restrictions: Dict[str, int]
) -> Tuple[bool, str]:
    """
    ...
    if "minimum" in restrictions and patient_age < restrictions["minimum"]:
        return (
            True,
            f"Patient age {patient_age} is below minimum age {restrictions['minimum']}",
        )

    if "maximum" in restrictions and patient_age > restrictions["maximum"]:
        return (
            True,
            f"Patient age {patient_age} exceeds maximum age {restrictions['maximum']}",
        )
...
def matches_weight_restriction(
    patient_weight: Optional[float], restrictions: Dict[str, float]
) -> Tuple[bool, str]:
    """
...
def check_specialization_needs(
    patient_data: PatientData, campus_data: Dict[str, Any]
) -> List[Tuple[str, str]]:
    """
            ...
            for phrase in phrases:
                ...
                if len(phrase) > 5 and phrase in patient_text:
                    matched_exclusions.append(
                        (dept, f"Matched exclusion criterion: {exclusion}")
                    )
                    break
        ...
        for condition in dept_data.get("conditions", []):
            ...
            if check_keyword_match(patient_text, condition_keywords):
                matched_exclusions.append(
                    (
                        dept,
                        f"Patient condition matches excluded {condition} in {dept} department",
                    )
                )
...
def check_department_exclusions(
    patient_data: PatientData, department_data: Dict[str, Any], department_name: str
) -> List[CampusExclusion]:
    """
    ...
    for condition in department_data.get("conditions", []):
        ...
        if condition == "cardiac":
            ...
            condition_terms.extend(
                ["breathing", "breath", "respiratory", "lungs", "pulmonary"]
            )
        elif condition == "neurological":
            condition_terms.extend(
                ["brain", "neuro", "stroke", "seizure", "mental status"]
            )
        ...
        if any(term in patient_text for term in condition_terms):
            ...
            for exclusion in department_data.get("exclusions", []):
                ...
                key_phrases = [
                    phrase.strip()
                    for phrase in re.split(r"[,;.]", exclusion.lower())
...
def check_exclusions(
    patient_data: PatientData, campus: HospitalCampus
) -> List[CampusExclusion]:
    """
    ...
    if campus_criteria.get("age_restrictions") and hasattr(patient_data, "age"):
        matches, reason = matches_age_restriction(
    ...
    if campus_criteria.get("weight_restrictions") and hasattr(
        patient_data, "weight_kg"
    ):
        matches, reason = matches_weight_restriction(
    ...
    for exclusion in campus_criteria.get("general_exclusions", []):
        ...
        key_phrases = [
            phrase.strip()
            for phrase in re.split(r"[,;.]", exclusion_lower)
            if len(phrase.strip()) > 5
        ]
    ...
    for dept_name, dept_data in campus_criteria.get("departments", {}).items():
        dept_exclusions = check_department_exclusions(
            patient_data, dept_data, dept_name
        )
    ...
    if met_exclusions and "campus_notes" in campus_criteria:
        for note_key, note_text in campus_criteria["campus_notes"].items():
            if (
                note_key
                in ["ac_exclusion_picu_accept", "community_exclusion", "clinical_team"]
