...
def extract_vital_signs(patient_data: PatientData) -> Dict[str, Any]:
    ...
    if 'respiratory_rate' in patient_data.extracted_data:
        try:
            ...
            pass
    elif 'vital_signs' in patient_data.extracted_data and 'respiratory_rate' in patient_data.extracted_data['vital_signs']:
        try:
            respiratory_rate = float(patient_data.extracted_data['vital_signs']['respiratory_rate'])
        except (ValueError, TypeError):
            pass
    ...
    if 'heart_rate' in patient_data.extracted_data:
        try:
            ...
            pass
    elif 'vital_signs' in patient_data.extracted_data and 'heart_rate' in patient_data.extracted_data['vital_signs']:
        try:
    ...
    if 'systolic_bp' in patient_data.extracted_data:
        try:
            ...
            pass
    elif 'vital_signs' in patient_data.extracted_data and 'systolic_bp' in patient_data.extracted_data['vital_signs']:
        try:
            systolic_bp = float(patient_data.extracted_data['vital_signs']['systolic_bp'])
        except (ValueError, TypeError):
            pass
    ...
    if 'diastolic_bp' in patient_data.extracted_data:
        try:
            ...
            pass
    elif 'vital_signs' in patient_data.extracted_data and 'diastolic_bp' in patient_data.extracted_data['vital_signs']:
        try:
            diastolic_bp = float(patient_data.extracted_data['vital_signs']['diastolic_bp'])
        except (ValueError, TypeError):
            pass
    ...
    if 'oxygen_saturation' in patient_data.extracted_data:
        try:
            ...
            pass
    elif 'vital_signs' in patient_data.extracted_data and 'oxygen_saturation' in patient_data.extracted_data['vital_signs']:
        try:
            oxygen_saturation = float(patient_data.extracted_data['vital_signs']['oxygen_saturation'])
        except (ValueError, TypeError):
            pass
    elif 'vital_signs' in patient_data.extracted_data and 'spo2' in patient_data.extracted_data['vital_signs']:
        try:
            oxygen_saturation = float(patient_data.extracted_data['vital_signs']['spo2'])
        except (ValueError, TypeError):
            pass
    ...
    if "respiratory effort is increased" in clinical_text.lower() or "increased work of breathing" in clinical_text.lower():
        ...
        for term in resp_terms:
            if term in clinical_text.lower():
                ...
                if idx >= 0:
                    ...
                    for desc in ["normal", "mild", "moderate", "severe", "increased", "labored"]:
    ...
    if 'gcs' in patient_data.extracted_data:
        try:
            ...
            pass
    elif 'vital_signs' in patient_data.extracted_data and 'gcs' in patient_data.extracted_data['vital_signs']:
        try:
    ...
    elif 'vital_signs' in patient_data.extracted_data and 'mental_status' in patient_data.extracted_data['vital_signs']:
    ...
    if 'capillary_refill' in patient_data.extracted_data:
        try:
            ...
            pass
    elif 'vital_signs' in patient_data.extracted_data and 'capillary_refill' in patient_data.extracted_data['vital_signs']:
        try:
            capillary_refill = float(patient_data.extracted_data['vital_signs']['capillary_refill'])
        except (ValueError, TypeError):
            pass
    else:
        # Try to extract from clinical text
            ...
            if idx >= 0:
                ...
                for i in range(idx + 15, min(idx + 30, len(text_lower))):
                    if text_lower[i].isdigit() or text_lower[i] == '.':
                        j = i
                        while j < len(text_lower) and (text_lower[j].isdigit() or text_lower[j] == '.'):
                            j += 1
                        try:
...
def determine_trauma_status(patient_data: PatientData) -> bool:
    ...
    trauma_keywords = [
        "trauma", "accident", "injury", "fracture", "collision", "fall", "mvc",
        "motor vehicle", "crash", "assault", "burn", "blast", "gunshot", "stab",
...
def determine_care_level(scores: Dict[str, Any]) -> Tuple[List[str], List[str]]:
    ...
    if scores["prism3"] != "N/A" and isinstance(scores["prism3"]["score"], int):
        ...
        if prism_score >= 10:
            care_levels.append("PICU")
            justifications.append(f"PRISM III score {prism_score} (High mortality risk)")
    
    # Check Queensland score
    if "queensland_trauma" in scores and scores["queensland_trauma"] != "N/A" and isinstance(scores["queensland_trauma"]["score"], int):
        qld_score = scores["queensland_trauma"]["score"]
        if qld_score >= 9:
            care_levels.append("PICU")
            justifications.append(f"Queensland Trauma score {qld_score} (High/Critical Risk)")
    elif "queensland_non_trauma" in scores and scores["queensland_non_trauma"] != "N/A" and isinstance(scores["queensland_non_trauma"]["score"], int):
        qld_score = scores["queensland_non_trauma"]["score"]
        if qld_score >= 7:
            care_levels.append("PICU")
            justifications.append(f"Queensland Non-Trauma score {qld_score} (High/Critical Risk)")
    ...
    if not care_levels:
        ...
        any_elevated = False
        if scores["pews"] != "N/A" and isinstance(scores["pews"]["score"], int) and scores["pews"]["score"] >= 2:
            any_elevated = True
