...
class HospitalSearch:
    ...
    def load_hospitals(self) -> None:
        ...
        try:
                ...
                with open(hospital_path, 'r') as f:
                    ...
                    for hospital in hospitals_data:
                        ...
                        self.hospitals_cache[name] = {
                            ...
                            "longitude": hospital.get("location", {}).get("longitude", 0),
            ...
            texas_hospitals = [
                {"name": "Memorial Hermann Houston", "address": "6411 Fannin St, Houston, TX 77030"},
                {"name": "Houston Methodist Hospital", "address": "6565 Fannin St, Houston, TX 77030"},
                {"name": "Dell Children's Medical Center", "address": "4900 Mueller Blvd, Austin, TX 78723"},
                {"name": "St. David's Medical Center Austin", "address": "919 E 32nd St, Austin, TX 78705"},
                {"name": "Children's Health Dallas", "address": "1935 Medical District Dr, Dallas, TX 75235"},
                {"name": "CHRISTUS Trinity Mother Frances", "address": "800 E Dawson St, Tyler, TX 75701"},
                {"name": "University Hospital San Antonio", "address": "4502 Medical Dr, San Antonio, TX 78229"},
                {"name": "Baylor Scott & White Temple", "address": "2401 S 31st St, Temple, TX 76508"}
            ]
            ...
            for hospital in texas_hospitals:
                if hospital["name"] not in self.hospitals_cache:
                    ...
                    try:
                        location = self.geolocator.geocode(hospital["address"], timeout=5)
                        if location:
                        ...
                        logger.warning(f"Could not geocode {hospital['name']}: {str(e)}")
    ...
    def search_hospitals(self, query: str) -> List[Dict]:
        ...
        for name, details in self.hospitals_cache.items():
            if query in name.lower() or (details.get("address", "") and query in details["address"].lower()):
                results.append({
                    "name": name,
