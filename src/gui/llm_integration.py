...
class LLMClassifier:
    ...
    def test_connection(self, api_url: Optional[str] = None, model: Optional[str] = None) -> tuple:
        """
        ...
        try:
            ...
            if self.model in self.available_models:
                ...
                return False, f"Model {self.model} not found in available models: {self.available_models}"
        except Exception as e:
            error_msg = str(e)
            ...
            if "Unauthorized" in error_msg:
                return False, "Authentication error. Check if LM Studio requires an API key."
            elif "Connection refused" in error_msg or "Failed to establish a connection" in error_msg:
                return False, "Could not connect to the server. Make sure LM Studio is running and the URL is correct."
            elif "not found" in error_msg.lower() and self.model in error_msg:
                return False, f"Model '{self.model}' was not found. Check if it's loaded in LM Studio."
            else:
    ...
    def _run_entity_extraction(self, text: str) -> Dict[str, Any]:
        ...
        user_prompt = f"Extract clinical information from this patient vignette:\n\n{text}"
    ...
    def _run_specialty_assessment(self, extracted_entities: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        try:
            # Make the API call - LM Studio doesn't support 'system' role, so combine into user message
            response = self.client.chat.completions.create(
                model=self.model,
                messages=[
                    {"role": "user", "content": f"{system_prompt}\n\n{user_prompt}"}
            ...
            if match:
                ...
                logger.warning("Failed to parse JSON from specialty assessment response")
                return {"identified_specialties_needed": []}
        except Exception as e:
            logger.error(f"Error in specialty assessment: {str(e)}")
            return {"identified_specialties_needed": []}
    
    def _run_exclusion_evaluation(self, extracted_entities: Dict[str, Any], exclusion_criteria: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if match:
                ...
                logger.warning("Failed to parse JSON from exclusion evaluation response")
                return {"exclusion_criteria_evaluation": []}
        except Exception as e:
            logger.error(f"Error in exclusion evaluation: {str(e)}")
            return {"exclusion_criteria_evaluation": []}
    
    def _run_final_recommendation(self, 
                               extracted_entities: Dict[str, Any], 
                               specialty_assessment: Dict[str, Any], 
                               exclusion_evaluation: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        """
        ...
        combined_data = {
            ...
            "exclusion_criteria_evaluation": exclusion_evaluation.get("exclusion_criteria_evaluation", []) if exclusion_evaluation else []
        }
        ...
        try:
            ...
            if match:
                ...
                logger.warning("Failed to parse JSON from final recommendation response")
                return {
    ...
    def process_text(self, text: str, human_suggestions: Optional[Dict] = None) -> Dict[str, Any]:
        """
        ...
        """
        # First try with the LLM prompt chain, if that fails fall back to rule-based processing
        try:
            logger.info(f"Processing text with {self.model} on {self.api_url} using multi-step prompting")
            ...
            exclusion_criteria_path = os.path.join(
                ...
                'data', 'exclusion_criteria_clean.json'
            )
            exclusion_criteria = {}
            try:
            ...
            if exclusion_criteria:
                exclusion_result = self._run_exclusion_evaluation(entity_result, exclusion_criteria)
            
            # Step 4: Final Recommendation
            recommendation_result = self._run_final_recommendation(entity_result, specialty_result, exclusion_result)
            
            # Combine results into the expected format for the application
            result = {
                "chief_complaint": entity_result.get("medical_problems", ["Unknown"])[0] if entity_result.get("medical_problems") else entity_result.get("symptoms", ["Unknown"])[0] if entity_result.get("symptoms") else "Unknown",
                "clinical_history": entity_result.get("medical_history", text[:200] + "..."),
                ...
                "keywords": entity_result.get("symptoms", []) + entity_result.get("medical_problems", []),
                "suggested_care_level": recommendation_result.get("recommended_care_level", "General"),
                "note": f"Generated by {self.model} using multi-step prompting",
                
                # New fields from the prompt chain
                "specialty_needs": specialty_result.get("identified_specialties_needed", []),
                "exclusion_matches": exclusion_result.get("exclusion_criteria_evaluation", []) if exclusion_result else [],
                "explainability": {
    ...
    def _fallback_process_text(self, text: str, human_suggestions: Optional[Dict] = None) -> Dict[str, Any]:
        """
        ...
        if not vital_signs:
            ...
            hr_match = re.search(r'(?:HR|heart rate|pulse)[:\s]*([0-9]{2,3})\b', text, re.IGNORECASE)
            if hr_match:
                vital_signs["hr"] = hr_match.group(1)
                
            bp_match = re.search(r'(?:BP|blood pressure)[:\s]*([0-9]{2,3})[\s/]*([0-9]{2,3})\b', text, re.IGNORECASE)
            if bp_match:
                vital_signs["bp"] = f"{bp_match.group(1)}/{bp_match.group(2)}"
                
            rr_match = re.search(r'(?:RR|resp|respiratory rate)[:\s]*([0-9]{1,2})\b', text, re.IGNORECASE)
            if rr_match:
                vital_signs["rr"] = rr_match.group(1)
                
            o2_match = re.search(r'(?:O2|SpO2|oxygen|sat)[:\s]*([0-9]{1,3})\s*%?\b', text, re.IGNORECASE)
            if o2_match:
                vital_signs["o2"] = o2_match.group(1) + "%"
                
            temp_match = re.search(r'(?:T|temp|temperature)[:\s]*([0-9]{2}(?:\.[0-9])?)\s*(?:C|F)?\b', text, re.IGNORECASE)
            if temp_match:
        ...
        if not keywords and not potential_conditions:
            ...
            pediatric_keywords = [
                "bronchiolitis", "rsv", "pneumonia", "asthma", "croup", "febrile", "sepsis",
                "dehydration", "seizure", "trauma", "fracture", "respiratory distress",
                "failure to thrive", "intubation", "ventilator", "shock", "meningitis"
            ]
        ...
        if human_suggestions and "care_level" in human_suggestions:
            ...
            if "nicu" in text_lower or "newborn" in text_lower or "neonate" in text_lower or "premature" in text_lower:
                suggested_care_level = "NICU"
            elif "picu" in text_lower or "pediatric icu" in text_lower:
                suggested_care_level = "PICU"
            elif "icu" in text_lower or "intensive care" in text_lower:
                suggested_care_level = "ICU"
            elif ("respiratory distress" in text_lower or "intubated" in text_lower or 
                  "ventilator" in text_lower or "shock" in text_lower or "sepsis" in text_lower):
                suggested_care_level = "ICU"
