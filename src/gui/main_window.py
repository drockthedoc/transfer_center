...
from PyQt5.QtWidgets import (
    ...
    QGroupBox, QFormLayout, QTabWidget, QRadioButton, QCheckBox,
    QMessageBox, QFileDialog, QSplitter, QScrollArea, QFrame,
    QSpinBox, QTimeEdit, QDoubleSpinBox, QSlider, QCompleter, QListWidget,
    QListWidgetItem, QAction, QStatusBar
)
from PyQt5.QtCore import Qt, QTime, pyqtSlot, QSettings, QSize, QStringListModel
from PyQt5.QtGui import QFont, QIcon, QTextCursor, QPixmap, QColor

# Import project components
from src.core.models import (
    PatientData, Location, TransportMode, Recommendation,
    TransferRequest, HospitalCampus, WeatherData, MetroArea
)
...
class TransferCenterMainWindow(QMainWindow):
    ...
    def __init__(self, parent=None):
        ...
        self.last_census_update = self.settings.value("census/last_update", "Never", str)
        self.census_file_path = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), 'data', 'current_census.csv')
        self.hospital_file_path = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), 'data', 'sample_hospital_campuses.json')
    ...
    def _init_ui(self):
        ...
        self.clinical_data_input.setPlaceholderText("Enter or paste clinical data here...")
        ...
        self.location_combo.addItems(["No preference", "Prefers Houston", "Prefers Austin"])
        ...
        self.additional_suggestions.setPlaceholderText("Enter any additional suggestions or notes...")
        ...
        self.transport_combo.addItems(["POV (Private Vehicle)", "Local EMS", "Kangaroo Crew"])
        ...
        self.eta_time.setTime(QTime.currentTime().addSecs(3600))  # Default to 1 hour from now
        ...
        hospital_search_widget.setFixedHeight(140)  # Significantly smaller but still functional
        
        # Make the sending facility line edit show the full selected hospital name
        self.sending_facility = QLineEdit()
        self.sending_facility.setMinimumWidth(300)  # Ensure enough width for longer hospital names
        self.sending_facility.setPlaceholderText("Enter sending facility name or address")
        ...
        search_input_layout.addWidget(self.sending_facility, 4)  # Give more space to the input field
        ...
        self.hospital_results.setMaximumWidth(400)  # Limit width to prevent horizontal expansion
        ...
        # Keep the coordinate fields as hidden variables but don't display them in the UI
        ...
        transport_layout.addWidget(kc_container)  # Add the fixed-size container instead of the group directly
        ...
        self.recommendation_output.setStyleSheet("background-color: #f5f5f5; color: #333333; font-size: 10pt;")
        ...
        self.explanation_output.setStyleSheet("background-color: #f5f5f5; color: #333333; font-size: 10pt;")
        ...
        self.llm_output.setStyleSheet("background-color: #f5f5f5; color: #333333; font-size: 10pt;")
        ...
        self.llm_model_combo.addItem("Loading models...")  # Placeholder until models are loaded
        ...
        self.transport_output.setStyleSheet("background-color: #f5f5f5; color: #333333; font-size: 10pt;")
    ...
    def _update_transport_ui(self, index):
        ...
        # Show the Kangaroo Crew mode options only when Kangaroo Crew is selected (index 2)
    ...
    def _search_hospital(self):
        ...
        if not query:
            QMessageBox.warning(self, "Search Error", "Please enter a hospital name or address to search")
            return
        ...
        if not results:
            ...
            if lat and lon:
                ...
                self.hospital_results.item(0).setData(Qt.UserRole, {
                    "name": query,
                    "latitude": lat,
                    "longitude": lon,
                    "address": query,
    ...
    def _update_census_data(self):
        ...
        try:
            ...
            if not os.path.exists(self.census_file_path):
                QMessageBox.warning(self, "Census Update", 
                                  f"Census file not found: {self.census_file_path}")
                return
            
            # Update census data
            update_success = update_census(self.census_file_path, self.hospital_file_path)
            ...
            if update_success:
                ...
                self.census_status.setText(f"Census last updated: {self.last_census_update}")
                
                QMessageBox.information(self, "Census Update", 
                                      "Hospital campus data successfully updated with current census information.")
            else:
                QMessageBox.warning(self, "Census Update", 
            ...
            QMessageBox.critical(self, "Census Update Error", 
    ...
    def _load_config(self):
        ...
        try:
            # Only attempt to update if the census file exists
            if os.path.exists(self.census_file_path):
                update_success = update_census(self.census_file_path, self.hospital_file_path)
                if update_success:
                    self.last_census_update = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                    self.settings.setValue("census/last_update", self.last_census_update)
                    logger.info(f"Census data updated on startup: {self.last_census_update}")
        except Exception as e:
            logger.error(f"Error updating census on startup: {str(e)}")
        
        # Load hospital and weather data
        try:
            with open(self.hospital_file_path, 'r') as f:
                hospitals_data = json.load(f)
                self.hospitals = [HospitalCampus(**h) for h in hospitals_data]
        ...
        # Load weather data
        weather_file = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), 'data', 'sample_weather_conditions.json')
        try:
    ...
    def _test_llm_connection(self):
        ...
        if model in ["Loading models...", "Error loading models", "No models found - suggestions:"]:
        ...
        try:
            ...
            self.llm_output.append(f"<p>Testing connection to {api_url} with model: {model}...</p>")
            ...
            if success:
                ...
                self.llm_output.append(f"<p style='color:green'><b>Connection successful!</b></p>")
                self.llm_output.append(f"<p>Response: {message}</p>")
            else:
                QMessageBox.warning(self, "Connection Failed", 
                                   f"Failed to connect to LM Studio API: {message}")
                self.llm_output.append(f"<p style='color:red'><b>Connection failed!</b></p>")
                self.llm_output.append(f"<p>Error: {message}</p>")
                self.llm_output.append(f"<p>Troubleshooting tips:</p>")
                self.llm_output.append("<ul>")
                self.llm_output.append("<li>Make sure LM Studio is running and the server is started</li>")
                self.llm_output.append("<li>Check that the URL format is correct (usually http://localhost:1234/v1)</li>")
                self.llm_output.append("<li>Verify the model is loaded in LM Studio</li>")
                self.llm_output.append("<li>Try refreshing the model list</li>")
                self.llm_output.append("</ul>")
        except Exception as e:
            QMessageBox.critical(self, "Connection Error", 
                                f"Error connecting to LM Studio API: {str(e)}")
    ...
    def _on_submit(self):
        ...
        if not self.sending_facility.text().strip():
            QMessageBox.warning(self, "Input Error", "Please enter sending facility name")
            return
        
        # Check if we have a valid model selected
        current_model = self.llm_model_combo.currentText()
        if current_model in ["Loading models...", "Error loading models", "No models found - suggestions:"]:
            # Try to refresh models first
            self._refresh_llm_models()
            
            # Check if we now have valid models
            if self.llm_model_combo.currentText() in ["Loading models...", "Error loading models", "No models found - suggestions:"]:
                response = QMessageBox.question(self, "Model Issue", 
        ...
        if model_name not in ["Loading models...", "Error loading models", "No models found - suggestions:"]:
        ...
        try:
            ...
            QMessageBox.warning(self, "Input Error", "Invalid coordinates. Please select a valid location.")
            return
        
        # Process with LLM
        llm_results = self.llm_classifier.process_text(clinical_text, 
                                                      human_suggestions=self._get_human_suggestions())
        
        # Display LLM results
        self.llm_output.clear()
        self.llm_output.append(f"<h3>LLM Classification Results</h3>")
        self.llm_output.append(f"<p><b>Chief Complaint:</b> {llm_results.get('chief_complaint', 'Not identified')}</p>")
        self.llm_output.append(f"<p><b>Clinical History:</b> {llm_results.get('clinical_history', 'Not identified')}</p>")
        ...
        if transport_type == "POV (Private Vehicle)":
            transport_modes = [TransportMode.GROUND_AMBULANCE]  # Use ground as proxy for POV
        ...
        transport_details = self.transport_estimator.estimate_transport_times(
            ...
            kc_mode="fixed_wing" if self.kc_fixed.isChecked() else 
                    "helicopter" if self.kc_rotor.isChecked() else "ground"
        )
        ...
        for hospital_id, details in transport_details.items():
            hospital_name = next((h.name for h in self.hospitals if h.campus_id == hospital_id), hospital_id)
            self.transport_output.append(f"<h4>{hospital_name}</h4>")
            self.transport_output.append(f"<p><b>Estimated Travel Time:</b> {details['time_minutes']} minutes</p>")
            self.transport_output.append(f"<p><b>Distance:</b> {details['distance_km']:.1f} km</p>")
            self.transport_output.append(f"<p><b>Transport Mode:</b> {details['mode']}</p>")
            
            if 'traffic_factor' in details:
                self.transport_output.append(f"<p><b>Traffic Factor:</b> {details['traffic_factor']:.2f}x</p>")
        ...
        try:
            ...
            QMessageBox.critical(self, "Recommendation Error", 
    ...
    def _display_recommendation(self, recommendation: Optional[Recommendation]):
        ...
        if not recommendation:
            ...
            self.recommendation_output.append("<p>The system could not determine a suitable recommendation based on the provided information.</p>")
            return
        
        # Find the recommended hospital
        recommended_hospital = next((h for h in self.hospitals if h.campus_id == recommendation.recommended_campus_id), None)
        hospital_name = recommended_hospital.name if recommended_hospital else recommendation.recommended_campus_id
        
        # Display main recommendation
        self.recommendation_output.append(f"<h2>Recommendation: {hospital_name}</h2>")
        self.recommendation_output.append(f"<p><b>Confidence:</b> {recommendation.confidence_score:.1f}%</p>")
        self.recommendation_output.append(f"<p><b>Reason:</b> {recommendation.reason}</p>")
        
        if recommended_hospital:
            self.recommendation_output.append("<h3>Hospital Details</h3>")
            self.recommendation_output.append(f"<p><b>Location:</b> {recommended_hospital.address}</p>")
            self.recommendation_output.append("<p><b>Bed Availability:</b></p><ul>")
            self.recommendation_output.append(f"<li>General: {recommended_hospital.bed_census.available_beds}/{recommended_hospital.bed_census.total_beds}</li>")
            self.recommendation_output.append(f"<li>ICU: {recommended_hospital.bed_census.icu_beds_available}/{recommended_hospital.bed_census.icu_beds_total}</li>")
            self.recommendation_output.append(f"<li>NICU: {recommended_hospital.bed_census.nicu_beds_available}/{recommended_hospital.bed_census.nicu_beds_total}</li>")
        ...
        if recommendation.explainability_details:
            ...
            if isinstance(recommendation.explainability_details, dict):
                for category, details in recommendation.explainability_details.items():
                    self.explanation_output.append(f"<h4>{category.replace('_', ' ').title()}</h4>")
                ...
                self.explanation_output.append(f"<p>{str(recommendation.explainability_details)}</p>")
