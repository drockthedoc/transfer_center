...
class TransferCenterMainWindow(QMainWindow):
    ...
    def _update_census_data(self):
        ...
        try:
            ...
            if updated_hospitals:
                ...
                self.census_widget.set_status(
                    "<p><b>Error updating census data.</b></p>"
                )
    ...
    def _load_config(self):
        ...
        try:
            ...
            if os.path.exists(self.hospital_file_path):
                ...
                for campus_data in hospital_data:
                    campus = HospitalCampus(
                        ...
                        metro_area=MetroArea(
                            campus_data.get("metro_area", "HOUSTON_METRO")
                        ),
                        address=campus_data.get("address", ""),
                        location=Location(
                            latitude=campus_data.get("location", {}).get("latitude", 0),
                            longitude=campus_data.get("location", {}).get(
                                "longitude", 0
                            ),
                        ),
                        bed_census={
                            "total_beds": campus_data.get("bed_census", {}).get(
                                "total_beds", 0
                            ),
                            "available_beds": campus_data.get("bed_census", {}).get(
                                "available_beds", 0
                            ),
                            "icu_beds_total": campus_data.get("bed_census", {}).get(
                                "icu_beds_total", 0
                            ),
                            "icu_beds_available": campus_data.get("bed_census", {}).get(
                                "icu_beds_available", 0
                            ),
                            "nicu_beds_total": campus_data.get("bed_census", {}).get(
                                "nicu_beds_total", 0
                            ),
                            "nicu_beds_available": campus_data.get(
                                "bed_census", {}
                            ).get("nicu_beds_available", 0),
                        },
                ...
                logger.warning(
                    f"Hospital data file not found: {self.hospital_file_path}"
                )
                self.statusBar.showMessage("Hospital data file not found")

            # Load LLM settings
            api_url = self.settings.value(
                "llm/api_url", "http://localhost:1234/v1", str
            )
    ...
    def _save_settings(self):
        ...
        self.settings.setValue(
            "llm/api_url", self.llm_settings_widget.api_url_input.text()
        )
        self.settings.setValue(
            "llm/model", self.llm_settings_widget.model_input.currentText()
        )
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    logger.info(
                        f"Generated pediatric scores: {list(scoring_results['scores'].keys())}"
                    )
                except Exception as score_error:
                    logger.error(
                        f"Error calculating pediatric scores: {str(score_error)}"
                    )
                    scoring_results = None

                # Process text with LLM, including scoring results if available
                extracted_data = self.llm_classifier.process_text(
                    clinical_data, scoring_results=scoring_results
                )
                ...
                if "final_recommendation" in display_data:
                    ...
                    display_data["recommendation_explanation"] = {
                        ...
                        "key_factors_for_recommendation": (
                            rec.explainability_details.get(
                                "key_factors_for_recommendation", []
                            )
                            if rec.explainability_details
                            else []
                        ),
                        ...
                        "other_considerations_from_notes": (
                            rec.explainability_details.get(
                                "other_considerations_from_notes", []
                            )
                            if rec.explainability_details
            ...
            patient = PatientData(
                ...
                care_level=(
                    extracted_data.get("suggested_care_level", "General")
                    if extracted_data
                    else "General"
                ),
            )
            ...
            if not extracted_data:
                raise ValueError(
                    "Clinical data extraction failed. Check LLM configuration."
                )
            ...
            if not recommendation:
                raise ValueError(
                    "LLM recommendation required but not available. Check logs for details."
                )

            # Ensure it's a proper Recommendation object
            if not isinstance(recommendation, Recommendation):
                raise ValueError(
                    f"Expected Recommendation object but got {type(recommendation).__name__}"
                )
            ...
            logger.info(
                f"Using LLM recommendation for {recommendation.recommended_campus_id} with confidence {recommendation.confidence_score}%"
            )
    ...
    def _display_recommendation(self, recommendation: Optional[Recommendation]):
        ...
        is_llm_recommendation = (
            recommendation.recommended_campus_id not in known_campus_ids
        )
        ...
        if "backup_campus" in details and details["backup_campus"]:
            ...
            recommendation_html += (
                f"<p><b>Backup Confidence:</b> {backup_confidence:.1f}%</p>"
            )
        ...
        recommendation_html += (
            f"<p><b>Recommended Care Level:</b> {care_level_display}</p>"
        )
        recommendation_html += (
            f"<p><b>Clinical Reasoning:</b> {recommendation.reason}</p>"
        )
        ...
        if "exclusions_checked" in details and details["exclusions_checked"]:
            ...
            for exclusion in details["exclusions_checked"]:
                ...
                if found:
                    recommendation_html += (
                        f"<li style='color: #ff0000;'><b>{name}:</b> FOUND ⚠️</li>"
                    )
                else:
        ...
        if "traffic_report" in details:
            recommendation_html += (
                f"<p><b>Traffic Conditions:</b> {details['traffic_report']}</p>"
            )

        # Weather Report
        if "weather_report" in details:
            recommendation_html += (
                f"<p><b>Weather Conditions:</b> {details['weather_report']}</p>"
            )
        ...
        if "campus_scores" in details:
            ...
            if "primary" in scores:
                ...
                recommendation_html += (
                    "<h4>Primary Recommendation Scores (1-5)</h4><ul>"
                )
                ...
                recommendation_html += (
                    f"<li><b>Capacity:</b> {primary_scores.get('capacity', 'N/A')}</li>"
                )
                recommendation_html += (
                    f"<li><b>Location:</b> {primary_scores.get('location', 'N/A')}</li>"
                )
                ...
                for key in [
                    ...
                    if key in primary_scores and isinstance(
                        primary_scores[key], (int, float)
                    ):
            ...
            if "backup" in scores:
                ...
                recommendation_html += (
                    f"<li><b>Capacity:</b> {backup_scores.get('capacity', 'N/A')}</li>"
                )
                recommendation_html += (
                    f"<li><b>Location:</b> {backup_scores.get('location', 'N/A')}</li>"
                )
        ...
        if (
            "transport_considerations" in details
            and details["transport_considerations"]
        ):
