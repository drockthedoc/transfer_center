    ...
    def evaluate_exclusions(
        ...
        try:
            ...
            response = self.client.chat.completions.create(
                model=self.model,
                messages=[
                    {"role": "system", "content": "You are a hospital transfer coordinator."},
...
"""
    ...
    def _fallback_evaluation(
        ...
        patient_weight = None
        if "demographics" in extracted_entities and "weight" in extracted_entities["demographics"]:
        ...
        if "clinical_info" in extracted_entities:
            if "chief_complaint" in extracted_entities["clinical_info"]:
                patient_text += extracted_entities["clinical_info"]["chief_complaint"] + " "
            if "clinical_history" in extracted_entities["clinical_info"]:
                patient_text += extracted_entities["clinical_info"]["clinical_history"] + " "
            if "diagnoses" in extracted_entities["clinical_info"]:
                if isinstance(extracted_entities["clinical_info"]["diagnoses"], list):
                    patient_text += " ".join(extracted_entities["clinical_info"]["diagnoses"]) + " "
        ...
        for campus_id, campus_data in exclusion_criteria.get("campuses", {}).items():
            ...
            for dept_id, dept_data in campus_data.get("departments", {}).items():
                ...
                if patient_age is not None and age_restrictions:
                    if "minimum" in age_restrictions and patient_age < age_restrictions["minimum"]:
                        matches.append({
                    ...
                    if "maximum" in age_restrictions and patient_age > age_restrictions["maximum"]:
                        matches.append({
                ...
                if patient_weight is not None and weight_restrictions:
                    if "minimum" in weight_restrictions and patient_weight < weight_restrictions["minimum"]:
                        matches.append({
                    ...
                    if "maximum" in weight_restrictions and patient_weight > weight_restrictions["maximum"]:
                        matches.append({
        ...
        if campus_matches:
            ...
            eligible_campuses = [
                campus_id for campus_id, count in campus_matches.items() 
            ...
            if eligible_campuses:
                ...
                if min_exclusions == 0:
                    result["recommendation_reasoning"] = f"Campus {eligible_campuses[0]} has no exclusion matches"
                else:
                    result["recommendation_reasoning"] = f"Campus {eligible_campuses[0]} has fewest exclusion matches ({min_exclusions})"
