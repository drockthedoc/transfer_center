    ...
    def assess_specialties(self, extracted_entities: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if assessment:
                ...
                logger.warning("Robust JSON parsing failed, falling back to rule-based assessment")
    ...
    def _fallback_assessment(self, extracted_entities: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "clinical_info" in extracted_entities:
            ...
            if "clinical_history" in extracted_entities["clinical_info"]:
                all_text += extracted_entities["clinical_info"]["clinical_history"] + " "
            if "diagnoses" in extracted_entities["clinical_info"]:
                if isinstance(extracted_entities["clinical_info"]["diagnoses"], list):
                    all_text += " ".join(extracted_entities["clinical_info"]["diagnoses"]) + " "
        ...
        # Cardiology
        if any(keyword in all_text for keyword in ["heart", "cardiac", "chest pain", "murmur", "arrhythmia", "tachycardia", "bradycardia"]):
            specialties.append({
                "specialty": "Cardiology",
                "importance": "primary" if "heart failure" in all_text or "cardiac arrest" in all_text else "secondary",
                "reasoning": "Cardiac symptoms or history mentioned"
            })
            result["potential_conditions"].append("Cardiac condition")
        
        # Pulmonology
        if any(keyword in all_text for keyword in ["respiratory", "breathing", "lung", "asthma", "pneumonia", "bronchiolitis", "cough"]):
            specialties.append({
                "specialty": "Pulmonology",
                "importance": "primary" if "respiratory distress" in all_text or "breathing difficulty" in all_text else "secondary",
                "reasoning": "Respiratory symptoms or history mentioned"
            })
            result["potential_conditions"].append("Respiratory condition")
        
        # Neurology
        if any(keyword in all_text for keyword in ["neurological", "seizure", "stroke", "brain", "headache", "altered mental status"]):
            specialties.append({
                "specialty": "Neurology",
                "importance": "primary" if "seizure" in all_text or "stroke" in all_text else "secondary",
                "reasoning": "Neurological symptoms or history mentioned"
            })
            result["potential_conditions"].append("Neurological condition")
        
        # Infectious Disease
        if any(keyword in all_text for keyword in ["infection", "fever", "sepsis", "meningitis"]):
            specialties.append({
                "specialty": "Infectious Disease",
                "importance": "primary" if "sepsis" in all_text or "meningitis" in all_text else "secondary",
        ...
        if "care_needs" in extracted_entities and "suggested_care_level" in extracted_entities["care_needs"]:
            result["recommended_care_level"] = extracted_entities["care_needs"]["suggested_care_level"]
        elif "vital_signs" in extracted_entities:
            vitals = extracted_entities["vital_signs"]
            # Check for critical vitals
            if any([
                "hr" in vitals and isinstance(vitals["hr"], (int, float)) and (vitals["hr"] > 180 or vitals["hr"] < 60),
                "rr" in vitals and isinstance(vitals["rr"], (int, float)) and (vitals["rr"] > 40 or vitals["rr"] < 10),
                "o2" in vitals and isinstance(vitals["o2"], str) and vitals["o2"].rstrip("%").isdigit() and int(vitals["o2"].rstrip("%")) < 90
            ]):
                result["recommended_care_level"] = "ICU"
                result["care_level_reasoning"] = "Critical vital signs detected"
        
        # Check NICU criteria
        if "demographics" in extracted_entities and "age" in extracted_entities["demographics"]:
