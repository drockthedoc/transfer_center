...
from src.llm.classification import parse_patient_text, determine_care_level
from src.core.exclusion_checker import load_exclusion_criteria, check_exclusions
...
class LLMInterface:
    ...
    def analyze_patient_for_transfer(self, 
                                    patient_vignette: str, 
                                    target_campuses: List[HospitalCampus]) -> Dict[str, Any]:
        """
        ...
        patient_data, extraction_source = self._extract_patient_data_with_fallback(patient_vignette)
        ...
        for campus in target_campuses:
            try:
                ...
                campus_analysis = self._analyze_campus_for_patient(patient_vignette, patient_data, campus)
        ...
        recommended_campus, care_level, explanation = self._determine_recommended_campus(
    ...
    def _extract_patient_data_with_fallback(self, patient_vignette: str) -> Tuple[PatientData, str]:
        """
        ...
        try:
            ...
            minimal_data = PatientData(
                ...
                clinical_history=patient_vignette[:500]  # Use truncated vignette as history
            )
            return minimal_data, "minimal_fallback"
    
    def _analyze_campus_for_patient(self, 
                                   patient_vignette: str, 
                                   patient_data: PatientData, 
                                   campus: HospitalCampus) -> Dict[str, Any]:
        """
        ...
        for excl in llm_analysis.get("exclusion_criteria_evaluation", []):
            if excl.get("status") == "likely_met":
                combined_exclusions.append({
                    "name": f"LLM-Identified: {excl.get('exclusion_rule_id', 'Unknown')}",
        ...
        for specialty in llm_analysis.get("identified_specialties_needed", []):
            if specialty.get("likelihood_score", 0) > 50:  # Only include likely specialties
    ...
    def _fallback_campus_analysis(self, patient_data: PatientData, campus: HospitalCampus) -> Dict[str, Any]:
        """
    ...
    def _determine_recommended_campus(self, 
                                     patient_data: PatientData, 
                                     campuses: List[HospitalCampus], 
                                     campus_analyses: List[Dict[str, Any]]) -> Tuple[Optional[HospitalCampus], str, str]:
        """
        ...
        if not suitable_campuses:
            ...
            explanation = (
                f"No fully suitable campus found. {recommended_campus.name} is recommended as the least " +
        ...
        if suitable_for_care_level:
            ...
            for i, campus in enumerate(suitable_for_care_level):
                for j, analysis in enumerate(campus_analyses):
                    if analysis["campus_id"] == campus.campus_id and analysis["confidence"] > best_confidence:
            ...
            explanation = (
                ...
                f"Patient requires {care_level} level care. " +
                (matching_analysis["explanation"] if matching_analysis and matching_analysis["explanation"] else "")
            )
        else:
