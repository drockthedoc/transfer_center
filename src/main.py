...
from rich import print as rprint # Use rprint to avoid conflict with built-in print if needed

from src.core.models import (
    HospitalCampus, WeatherData, PatientData, TransferRequest, Location, TransportMode
)
...
def recommend_transfer(
    ...
    hospitals_file: Path = typer.Option(
        "data/sample_hospital_campuses.json", "--hospitals", "-h",
        help="Path to hospital data JSON file.",
        exists=True, file_okay=True, dir_okay=False, readable=True, resolve_path=True
    ),
    weather_file: Path = typer.Option(
        "data/sample_weather_conditions.json", "--weather", "-w",
    ...
    chief_complaint: Optional[str] = typer.Option(None, "--complaint", help="Patient's chief complaint (if not using files or to override notes)."),
    clinical_history: Optional[str] = typer.Option(None, "--history", help="Patient's clinical history (if not using files or to override notes)."),
    sending_facility_name: str = typer.Option("Sending General CLI", "--sfn", help="Name of the sending facility."),
    sending_lat: float = typer.Option(29.7300, "--slat", help="Latitude of sending facility."),
    sending_lon: float = typer.Option(-95.3800, "--slon", help="Longitude of sending facility."),
    transport_modes_csv: str = typer.Option(
        "GROUND_AMBULANCE,AIR_AMBULANCE", "--transport",
    ...
    """
    console = Console()
    rprint(f"[bold cyan]Starting recommendation process for patient: {patient_id}[/bold cyan]")
    ...
    try:
        ...
        rprint(f":sun_behind_cloud: Loaded weather data (using first entry) from [italic]{weather_file}[/italic]")
    ...
    if patient_notes_file:
        rprint(f":page_facing_up: Processing patient notes from [italic]{patient_notes_file}[/italic]...")
        try:
            notes_content = patient_notes_file.read_text()
            llm_output = parse_patient_text(notes_content)
            
            # Prioritize CLI input for complaint/history if provided, else use LLM summary
            final_complaint = chief_complaint if chief_complaint else llm_output.get("raw_text_summary", "Summary unavailable from notes.")
            ...
            final_history = clinical_history if clinical_history else "Clinical history not detailed in notes summary or CLI."
            
            llm_derived_complaint = llm_output.get("raw_text_summary", "Summary unavailable.")
            ...
            rprint(f"  [green]:heavy_check_mark: Patient data constructed from notes file. LLM summary for complaint: '{llm_derived_complaint}'[/green]")
            if chief_complaint: rprint(f"  [yellow]:information_source: Used CLI override for chief complaint.[/yellow]")
            if clinical_history: rprint(f"  [yellow]:information_source: Used CLI override for clinical history.[/yellow]")
        ...
        rprint(f":page_facing_up: Loading patient data from [italic]{patient_data_file}[/italic]...")
        try:
            ...
            rprint(f"[bold red]:x: Error loading structured patient data file: {e}[/bold red]")
            raise typer.Exit(code=1)
        ...
        if not chief_complaint or not clinical_history:
            rprint("[bold yellow]:warning: Chief complaint or clinical history not provided directly via CLI. Using placeholders or defaults. Consider using --notes-file or --patient-file for more complete data.[/bold yellow]")
        
        patient_data_obj = PatientData(
            patient_id=patient_id,
            chief_complaint=chief_complaint if chief_complaint else "Not specified via CLI.",
            clinical_history=clinical_history if clinical_history else "Not specified via CLI.",
            vital_signs={}, # No direct CLI input for these in this example
            labs={},
            current_location=sending_facility_location
        )
        rprint(f"  [green]:heavy_check_mark: Patient data constructed from CLI arguments.[/green]")

    if not patient_data_obj: # Should not happen if logic is correct, but as a safeguard
    ...
    if not parsed_transport_modes: # Default if none valid or provided
        ...
        rprint(f"[yellow]:information_source: No valid transport modes provided or parsed. Defaulting to: {parsed_transport_modes}[/yellow]")
    rprint(f":ambulance::helicopter: Using available transport modes: {[mode.value for mode in parsed_transport_modes]}")
    ...
    rprint(f":clipboard: Transfer request created with ID: {transfer_request.request_id}")
    ...
    if recommendation:
        console.print(f"[bold green]Recommendation for Request ID: {recommendation.transfer_request_id}[/bold green]")
        
        table = Table(title="Recommendation Details", show_header=True, header_style="bold blue")
        ...
        table.add_row("Recommended Campus", f"{recommended_campus_name} (ID: {recommendation.recommended_campus_id})")
        table.add_row("Reason", recommendation.reason)
        table.add_row("Confidence Score", f"{recommendation.confidence_score:.2f}" if recommendation.confidence_score is not None else "N/A")
        
        console.print(table)
        
        if recommendation.explainability_details:
            console.print("\n[bold]Explainability Details:[/bold]")
            console.print(JSON(json.dumps(recommendation.explainability_details, indent=2)))
        ...
        console.print("[bold red]:x: No suitable campus found for the given criteria.[/bold red]")
