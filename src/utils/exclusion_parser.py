...
CONDITION_KEYWORDS = {
    ...
    "trauma": ["trauma", "injury", "accident", "fracture", "burn", "wound"],
    "transplant": ["transplant", "donor", "graft", "rejection", "immunosuppression"],
    "endocrine": [
        "diabetes",
        "diabetic",
        "dka",
    ...
    "surgery": ["surgery", "surgical", "post-op", "operative", "incision", "procedure"],
    "gi": [
        "gastrointestinal",
        "gi",
        "liver",
...
def parse_austin_exclusions(text: str) -> Dict[str, Any]:
    ...
    for section in sections:
        ...
        for item in bullet_items:
            ...
            if "Administrative Review" in section or "general" in department:
                ...
                dept = (
                    identify_department(item) if "general" in department else department
                )
    ...
    for dept in result["departments"]:
        result["departments"][dept]["conditions"] = list(
            result["departments"][dept]["conditions"]
        )
...
def process_community_table_section(section: str, result: Dict[str, Any]) -> None:
    ...
    """
    # Identify which department this section belongs to by looking at the
    # first few lines
    header_lines = section.split("\n", 3)[
        0:2
    ]  # Get first two lines for department identification
    ...
    if department not in result["departments"]:
        ...
        result["departments"][department]["conditions"].update(
            identify_conditions(section)
        )
    ...
    community_exclusions = re.findall(
        r"(?:^|\n)\s*(?:•|\*|\-|\u2022|\uf0b7)?\s*([^\n]+?)\s+(?:X|✓)\s+(?:Community)",
        section,
        re.DOTALL,
    )
    ...
    for dept_key, dept_name in departments.items():
        ...
        if match:
            ...
            if dept_key not in result["departments"]:
                ...
                result["departments"][dept_key]["conditions"].update(
                    identify_conditions(section_text)
                )
            ...
            for item in items:
                ...
                if (
                    len(item) > 5
                    and item not in result["departments"][dept_key]["exclusions"]
                ):
...
def clean_exclusion_data(result: Dict[str, Any]) -> None:
    ...
    for dept, dept_data in result["departments"].items():
        ...
        for i, item in enumerate(dept_data["exclusions"]):
            ...
            # Check the next item (if available) to see if it starts lowercase or with
            # a continuation indicator
            if i + 1 < len(dept_data["exclusions"]):
                next_item = dept_data["exclusions"][i + 1]
                if (next_item and next_item[0].islower()) or next_item.startswith(
                    ("with", "without", "or", "and", "requiring")
                ):
