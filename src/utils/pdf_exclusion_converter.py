...
def extract_text_from_pdf(pdf_path: str) -> str:
    ...
    try:
        ...
        try:
            ...
            logger.error(
                "PyPDF2 not available. Please install with 'pip install PyPDF2'"
            )
            return ""
...
def parse_exclusion_criteria(text: str) -> List[Dict[str, Any]]:
    ...
    # This is a simplified approach and may need refinement based on actual
    # PDF structure
    sections = re.split(r"\n\s*\d+[\.|\)]\s+", text)
    ...
    for section in sections:
        ...
        age_match = re.search(
            r"(\d+)\s*(?:years|yo|y\.o\.|year)", section, re.IGNORECASE
        )
        age_limit = int(age_match.group(1)) if age_match else None

        # Check for weight restrictions
        weight_match = re.search(
            r"(\d+)\s*(?:kg|kilograms|pounds|lbs)", section, re.IGNORECASE
        )
        ...
        criteria.append(
            {
                ...
                "keywords": [
                    word.lower()
                    for word in re.findall(r"\b\w{4,}\b", section)
                    if word.lower()
                    not in ["with", "that", "this", "from", "they", "have", "will"]
                ],
            }
        )
