...
class TransportTimeEstimator:
    ...
    def _calculate_kangaroo_crew_time(
        ...
        sender_to_receiver_distance = calculate_distance(sending_location, receiving_location)
        
        # Get speed for the selected mode
        speed = self.speeds.get(mode, self.speeds["ground"])
        
        # Calculate travel times
        base_to_sender_time = calculate_direct_travel_time(base_to_sender_distance, speed)
        sender_to_receiver_time = calculate_direct_travel_time(sender_to_receiver_distance, speed)
        ...
        time_until_second_leg = 30 + base_to_sender_time  # 30 min prep + travel to sender
        
        # Get traffic factor for the second leg
        traffic_factor2 = get_traffic_factor(dest_metro_area, int(time_until_second_leg))
        ...
        total_time = prep_time + base_to_sender_time + patient_prep_time + sender_to_receiver_time
    ...
    def estimate_transport_times(
        ...
        for hospital in hospitals:
            ...
            if transport_type == "Kangaroo Crew":
                ...
                logger.debug(f"Kangaroo Crew time to {hospital.name}: {time_minutes:.1f} minutes")
            ...
            # Otherwise, calculate times for each available transport mode
                ...
                for mode in transport_modes:
                    ...
                    logger.debug(f"{mode_str} time to {hospital.name}: {time_minutes:.1f} minutes")
