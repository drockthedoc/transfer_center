...
def get_road_travel_info(origin: Location, destination: Location) -> Dict[str, any]:
    ...
    url = OSRM_API_URL.format(
        lon1=origin.longitude, lat1=origin.latitude,
        lon2=destination.longitude, lat2=destination.latitude
    )
    ...
    try:
        ...
        if data.get('routes') and data['routes'][0]: # Check if 'routes' is not empty and has at least one route
            route = data['routes'][0]
            ...
            if distance_m is not None and duration_s is not None:
                #print(f"OSRM API success for {origin} to {destination}") # Optional: for worker debugging
                return {
                ...
                print(f"Warning: OSRM API route for {origin} to {destination} is incomplete. Using fallback.")
        else:
            # API returned success but no route found (e.g. 'routes' was empty or not present)
            print(f"Warning: OSRM API found no route between {origin} and {destination}. Using fallback.")
    
    except requests.exceptions.Timeout:
        print(f"Warning: OSRM API request timed out for route {origin} to {destination}. Using fallback.")
        ...
        print(f"Warning: OSRM API request failed due to connection error for route {origin} to {destination}. Using fallback.")
    except requests.exceptions.RequestException as e: # Catch-all for other requests issues
        print(f"Warning: OSRM API request failed for route {origin} to {destination}: {e}. Using fallback.")
    except (KeyError, IndexError, ValueError) as e: # For issues with JSON parsing (e.g. data['routes'][0] fails)
        print(f"Warning: Error parsing OSRM API response for route {origin} to {destination}: {e}. Using fallback.")

    # Fallback mechanism
    #print(f"Executing fallback for {origin} to {destination}") # Optional: for worker debugging
    ...
    estimated_time_min = (haversine_dist_km / DEFAULT_AVERAGE_SPEED_KMH) * 60 if DEFAULT_AVERAGE_SPEED_KMH > 0 else 0 
...
ADVERSE_WEATHER_FOR_AIR_TRAVEL = ["FOG", "THUNDERSTORM", "HIGH_WINDS", "BLIZZARD", "FREEZING_RAIN", "HURRICANE", "SEVERE TURBULENCE"]
...
def get_air_travel_info(
    ...
    if weather.adverse_conditions:
        for condition in weather.adverse_conditions:
            ...
            if condition.upper() in [adv_cond.upper() for adv_cond in ADVERSE_WEATHER_FOR_AIR_TRAVEL]:
                return {"viable": False, "reason": f"Adverse weather: {condition}.", "distance_km": 0, "time_minutes": 0, "source": "Air Travel Calculation"}
    
    if weather.visibility_km < MIN_VISIBILITY_KM_VFR:
        return {"viable": False, "reason": f"Visibility {weather.visibility_km}km < minimum {MIN_VISIBILITY_KM_VFR}km for VFR.", "distance_km": 0, "time_minutes": 0, "source": "Air Travel Calculation"}

    if weather.wind_speed_kph > MAX_WIND_SPEED_KPH_AIR:
        return {"viable": False, "reason": f"Wind speed {weather.wind_speed_kph}kph > maximum {MAX_WIND_SPEED_KPH_AIR}kph.", "distance_km": 0, "time_minutes": 0, "source": "Air Travel Calculation"}
    ...
    if average_helicopter_speed_kmh <= 0:
        return {"viable": False, "reason": "Average helicopter speed must be positive.", "distance_km": round(distance_km, 2), "time_minutes": float('inf'), "source": "Air Travel Calculation"}
    ...
    return {"viable": True, "reason": "Weather conditions suitable for air travel.", "distance_km": round(distance_km, 2), "time_minutes": round(total_time_minutes, 2), "source": "Air Travel Calculation"}
