...
class TestPediatricExclusionChecker(unittest.TestCase):  # Renamed class
    ...
    def setUp(self):
        ...
        self.campus_specialized_peds = HospitalCampus(
            campus_id="TCH_MAIN_SIM",
            name="Peds Speciality Hospital",
            metro_area=MetroArea.HOUSTON,
            address="",
            location=Location(latitude=0, longitude=0),
            exclusions=[
                self.exclusion_no_neonatal_surgery,
                self.exclusion_no_complex_cardiac_peds,
                self.exclusion_adults_only_general,
            ],
        ...
        self.campus_general_no_peds_exclusions = HospitalCampus(
            campus_id="GEN_HOSP_SIM",
            name="General Hospital (No Peds Exclusions)",
            metro_area=MetroArea.AUSTIN,
            address="",
            location=Location(latitude=0, longitude=0),
            exclusions=[
                self.exclusion_adults_only_general
            ],  # Only has an adult exclusion
            bed_census=BedCensus(
                total_beds=10,
                available_beds=1,
                icu_beds_total=1,
    ...
    def test_neonate_meets_no_neonatal_surgery_exclusion(self):
        ...
        self.patient_neonate.chief_complaint = (
            "Neonate surgery needed for PDA ligation."
        )
    ...
    def test_child_cardiac_meets_no_complex_cardiac_exclusion_history(self):
        ...
        self.patient_child_cardiac.clinical_history = (
            "Patient has a congenital heart defect repair needed."
        )
        ...
        self.patient_child_cardiac.clinical_history = (
            "Scheduled for cardiac surgery consult"  # Reset
        )

    def test_teen_no_pediatric_exclusions_met(self):
        met = check_exclusions(
            self.patient_teen_no_issues, self.campus_specialized_peds
        )
        self.assertEqual(
            len(met),
            0,
            "Teenager with minor issue should not meet these specific peds surgery exclusions.",
        )

    def test_neonate_no_exclusions_at_general_hospital(self):
        # Neonate patient should not trigger the "Adults Only" exclusion at the
        # general hospital
        met = check_exclusions(
            self.patient_neonate, self.campus_general_no_peds_exclusions
        )
    ...
    def test_multiple_pediatric_exclusions_if_applicable(self):
        ...
        self.patient_child_cardiac.chief_complaint = (
            "Neonate surgery for complex heart defect repair needed."
        )
        self.patient_child_cardiac.clinical_history = (
            "Also requires neonatal operation for other issue."
        )
        # Re-add affected_keywords_in_history for exclusion_no_neonatal_surgery
        # for this test
        original_keywords_hist_neo_surg = (
            self.exclusion_no_neonatal_surgery.affected_keywords_in_history
        )
        self.exclusion_no_neonatal_surgery.affected_keywords_in_history = [
            "requires neonatal operation"
        ]
        ...
        self.patient_child_cardiac.chief_complaint = (
            "5yo with palpitations, known complex congenital heart defect"
        )
        self.patient_child_cardiac.clinical_history = (
            "Scheduled for cardiac surgery consult"
        )
        self.exclusion_no_neonatal_surgery.affected_keywords_in_history = (
            original_keywords_hist_neo_surg
        )
